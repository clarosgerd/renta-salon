...
@section('titulo', 'Editar salón')

@section('content')
    <div class="max-w-3xl">
        <div class="mb-5">
            <a href="{{ route('admin.salones.index') }}" class="text-sm text-gray-500 hover:underline">&larr; Salones</a>
            <h1 class="text-lg font-medium text-gray-900 mt-1">Editar salón — {{ $salon->nombre }}</h1>
        </div>

        <form method="POST" action="{{ route('admin.salones.update', $salon) }}" enctype="multipart/form-data"
