...
@push('scripts')
@if (session('abono_qr_id'))
<script>
    // Polling simple: consulta el estado del QR cada 3s hasta que se confirme.
    (function () {
        const abonoId = {{ session('abono_qr_id') }};
        const intervalo = setInterval(async () => {
            const res = await fetch(`/admin/pagos/${abonoId}/estado-qr`);
            if (!res.ok) return;
